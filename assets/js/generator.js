...
const Generator = {
  ...
  async build(config) {
    ...
    const pageIds = ['academic_setup','students','staff','classes','subjects','attendance','results','timetable','sow','cbt','assignments','library','conduct','health','promotion','fees','finance','leave','visitors','transport','announcements','events','messages','inbox','complaints','broadcast','gallery','eresources','birthdays','idcards','reports','directory','departments','parents','admissions','hr','payroll','staff_loans','staff_bonus','appraisals','rubrics','transcripts','transfer_cert','counselling','hostel','alumni','inventory','certificates','analytics','lms','gamification','cafeteria','financial_aid','front_desk','career_counseling','document_builder','fleet_tracking','facility_booking','compliance',
      ...
      'attendance','teacher-overview','messages','timetable','timetable-generator','checkin','diary','surveys','menu','settings','approvals','student-profile'];
...
const CBTUI = {
  ...
  promptGuide(){
    const txt = [
      ...
      'Return DOWNLOADABLE CSV only. Do not add markdown, numbering, commentary, code fences, or explanations outside the CSV.',
...
const Tools = {
  ...
  insertSymbol(x){ const el=this.active || document.activeElement; if(el && ('value' in el)){ const a=el.selectionStart||0,b=el.selectionEnd||0; el.value=el.value.slice(0,a)+x+el.value.slice(b); el.focus(); el.selectionStart=el.selectionEnd=a+x.length; if(typeof Exam!=='undefined') Exam.answer(el.value); } },
  calcPress(b){
  ...
  eval(){ try{ let e=this.expr.replace(/pi/g,'Math.PI').replace(/\be\b/g,'Math.E').replace(/sin\(/g,'Math.sin(').replace(/cos\(/g,'Math.cos(').replace(/tan\(/g,'Math.tan(').replace(/sqrt\(/g,'Math.sqrt(').replace(/log\(/g,'Math.log10(').replace(/ln\(/g,'Math.log('); while(/(\d+(?:\.\d+)?)\^(\d+(?:\.\d+)?)/.test(e)) e=e.replace(/(\d+(?:\.\d+)?)\^(\d+(?:\.\d+)?)/g,'Math.pow($1,$2)'); const val=Function('"use strict";return ('+e+')')(); this.ans=String(Math.round(Number(val)*1e10)/1e10); this.expr=this.ans; this.show(); }catch(err){ this.expr='Error'; this.show(); } }
};
Tools.init();
...
const EN={ rows:[], exam:null,
  ...
  certHTML(r){ var sc=(window.SCHOOL||{}); return '<div style="width:760px;max-width:96vw;border:10px double '+(sc.primary||'#4f46e5')+';padding:38px;text-align:center;font-family:Georgia,serif">'+
  ...
  printOne(html,title){ var sig=localStorage.getItem('sc-signature-url')||'', pn=localStorage.getItem('sc-principal-name')||'Principal'; var sign=sig?'<div style="text-align:center;margin-top:18px"><img src="'+sig+'" style="max-width:150px;max-height:70px;object-fit:contain"><br><b>'+pn+'</b><br>Official Signature</div>':''; var w=window.open('','_blank'); w.document.write('<html><head><title>'+title+'</title></head><body style="display:flex;justify-content:center;padding:20px"><div>'+html+sign+'</div><script>window.onload=()=>window.print()<\\/script></body></html>'); w.document.close(); },
  printMany(htmls,title){ var sig=localStorage.getItem('sc-signature-url')||'', pn=localStorage.getItem('sc-principal-name')||'Principal'; var sign=sig?'<div style="text-align:center;margin-top:18px"><img src="'+sig+'" style="max-width:150px;max-height:70px;object-fit:contain"><br><b>'+pn+'</b><br>Official Signature</div>':''; var w=window.open('','_blank'); w.document.write('<html><head><title>'+title+'</title></head><body>'+htmls.map(h=>'<div style="page-break-after:always;display:flex;justify-content:center;padding:14px"><div>'+h+sign+'</div></div>').join('')+'<script>window.onload=()=>window.print()<\\/script></body></html>'); w.document.close(); },
...
const AR={ rows:[],
 ...
 print(){ const el=document.getElementById('ar-output'); const w=window.open('','_blank'); var sig=localStorage.getItem('sc-signature-url')||'', pn=localStorage.getItem('sc-principal-name')||'Principal'; var sign=sig?'<div style="margin-top:35px;text-align:center"><img src="'+sig+'" style="max-width:160px;max-height:70px;object-fit:contain"><br><b>'+pn+'</b><br>Official Signature</div>':''; w.document.write('<html><head><title>Academic Record</title><style>body{font-family:Arial,sans-serif;padding:20px;color:#111}.print-record{text-align:center}table{width:100%;border-collapse:collapse;margin-top:12px}th,td{border:1px solid #111;padding:5px;font-size:12px}th{background:#eee}@media print{button{display:none}}</style></head><body>'+el.innerHTML+sign+'<script>window.onload=()=>window.print()<\\/script></body></html>'); w.document.close(); }
};
</script></body></html>`);
  },
...
document.addEventListener('DOMContentLoaded',ST.init); ST.init();
  ...
  pageParents(cfg) {
    const body = `
      <div class="card" style="margin-bottom:16px;background:#eef2ff;border-color:#c7d2fe">
        <h3 style="margin-top:0">👨‍👩‍👧 Parent / Guardian Registry</h3>
        <p style="color:var(--gray-700);line-height:1.7;margin:0">Use this dedicated admin page to create, view and edit parent/guardian records and link them to children. Parent logins are still created through Supabase Auth/signup for security; this page stores the school-side profile and mapping so the parent dashboard can show the correct child.</p>
      </div>
      <div class="grid grid-2">
        <div class="card"><h3>Parent records</h3><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px"><button class="btn btn-primary" onclick="PR.openParent()">+ Add parent</button><button class="btn btn-outline" onclick="PR.loadParents()">Refresh</button></div><div id="pr-parents">Loading…</div></div>
        <div class="card"><h3>Link parent to child</h3><div class="form-group"><label>Parent</label><select class="form-select" id="pr-parent"></select></div><div class="form-group"><label>Student</label><select class="form-select" id="pr-student"></select></div><div class="form-group"><label>Relationship</label><select class="form-select" id="pr-rel"><option>parent</option><option>father</option><option>mother</option><option>guardian</option><option>sponsor</option></select></div><button class="btn btn-primary" onclick="PR.link()">Link parent to child</button><div id="pr-links" style="margin-top:12px"></div></div>
      </div>`;
    return T.shell(cfg, 'Parents', body, { requireRole:'admin' }).replace('</body></html>', `<script>
const PR={parents:[],students:[],links:[],
 async init(){await Promise.all([this.loadParents(),this.loadStudents(),this.loadLinks()]);},
 async loadParents(){ if(!sb)return; const {data}=await sb.from('profiles').select('id,full_name,email,phone,role,status,member_id').eq('role','parent').order('full_name'); this.parents=data||[]; this.renderParents(); this.fill();},
 ...
 renderParents(){ const box=document.getElementById('pr-parents'); if(!box)return; box.innerHTML=this.parents.length?'<div class="table-wrap"><table><thead><tr><th>Name</th><th>Email/Login</th><th>Phone</th><th>Status</th><th>Edit</th></tr></thead><tbody>'+this.parents.map(p=>'<tr><td>'+esc(p.full_name||'')+'</td><td>'+esc(p.email||'')+'</td><td>'+esc(p.phone||'')+'</td><td><span class="badge">'+esc(p.status||'active')+'</span></td><td><button class="btn btn-sm btn-outline" onclick="PR.openParent(\\''+p.id+'\\')">Edit</button></td></tr>').join('')+'</tbody></table></div>':'<p>No parent records yet.</p>';},
 renderLinks(){ const box=document.getElementById('pr-links'); if(!box)return; box.innerHTML=this.links.length?'<div class="table-wrap"><table><thead><tr><th>Parent</th><th>Student</th><th>Relationship</th></tr></thead><tbody>'+this.links.slice(0,50).map(l=>{const p=this.parents.find(x=>x.id===l.parent_id)||{}, st=this.students.find(x=>x.id===l.student_id)||{}; return '<tr><td>'+esc(p.full_name||l.parent_id)+'</td><td>'+esc(st.full_name||l.student_id)+'</td><td>'+esc(l.relationship||'parent')+'</td></tr>';}).join('')+'</tbody></table></div>':'<p>No links yet.</p>';},
 openParent(id){ const p=this.parents.find(x=>x.id===id)||{}; openModal((id?'Edit':'Add')+' parent','<div class="form-group"><label>Full name</label><input class="form-input" id="pm-name" value="'+esc(p.full_name||'')+'"></div><div class="form-group"><label>Email / login email</label><input class="form-input" id="pm-email" type="email" value="'+esc(p.email||'')+'"></div><div class="form-group"><label>Phone</label><input class="form-input" id="pm-phone" value="'+esc(p.phone||'')+'"></div><div class="form-group"><label>Status</label><select class="form-select" id="pm-status"><option>active</option><option>pending</option><option>suspended</option></select></div><p style="color:var(--gray-600)">Login note: ask the parent to use this email on the Login page → Request access. Admin then approves the Supabase auth profile. Static sites cannot safely create passwords directly without a server/service-role key.</p>','<button class="btn btn-outline" onclick="closeModal()">Cancel</button><button class="btn btn-primary" onclick="PR.saveParent(\\''+(id||'')+'\\')">Save</button>'); setTimeout(()=>{const st=document.getElementById('pm-status'); if(st)st.value=p.status||'active';},0);},
 async saveParent(id){ const row={full_name:document.getElementById('pm-name').value.trim(),email:document.getElementById('pm-email').value.trim(),phone:document.getElementById('pm-phone').value.trim(),role:'parent',status:document.getElementById('pm-status').value}; if(!row.email){toast('Parent email/login email is required','warning');return;} if(!id){ const invite='Parent login invitation for '+row.full_name+'\nEmail: '+row.email+'\nInstruction: open login.html, click Request access, choose Parent, create password, then admin approves in Approvals.'; try{navigator.clipboard&&navigator.clipboard.writeText(invite);}catch(e){} openModal('Parent login invitation','<p>This static site cannot safely create an auth password without a server/service-role key. Send this invitation to the parent, then approve the account after signup.</p><textarea class="form-input" rows="6">'+esc(invite)+'</textarea>','<button class="btn btn-primary" onclick="closeModal()">OK</button>'); return;} const {error}=await sb.from('profiles').update(row).eq('id',id); if(error){toast(error.message,'danger');return;} closeModal(); toast('Parent saved','success'); this.loadParents();},
 async link(){ const row={parent_id:document.getElementById('pr-parent').value,student_id:document.getElementById('pr-student').value,relationship:document.getElementById('pr-rel').value}; if(!row.parent_id||!row.student_id){toast('Pick parent and student','warning');return;} const {error}=await sb.from('parent_child').upsert(row,{onConflict:'parent_id,student_id'}); if(error){toast(error.message,'danger');return;} toast('Linked','success'); this.loadLinks();}
